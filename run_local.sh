#!/usr/bin/env bash
# ============================================================
# InterfaceScout 2.0 - first-time setup (macOS / Linux)
# ============================================================
set -e
...
if ! python -c "import apbs_binary" >/dev/null 2>&1; then
  python -m pip install "apbs-binary>=3.4.1.2" || echo "WARNING: optional APBS binary could not be installed."
fi
...
if [ -d "$DESKTOP" ]; then
  if [ "$OS" = "Darwin" ]; then
    ...
    { echo '#!/usr/bin/env bash'; printf 'exec "%s/start.command"\n' "$PROJ_DIR"; } > "$LAUNCHER"
    chmod +x "$LAUNCHER"
  else
...
Type=Application
Name=InterfaceScout
Comment=Protein-material interface compatibility mapping
...
cd "$PROJ_DIR"
if [ "$OS" = "Darwin" ]; then exec "$PROJ_DIR/start.command"; else exec "$PROJ_DIR/start.sh"; fi
