...
The program prepares the selected protein structure once and calculates 11 canonical surface-chemistry maps. Candidate surface patches are ranked using chemistry support, side-chain accessibility, spatial coherence, and coarse orientation coherence without material-specific fitted weights.
...
The browser application provides two downloadable outputs:
...
The Excel workbook contains the sheets `Run_Method`, `Map_Definitions`, `Residue_Values`, `Patches`, and `Patch_Members`. A `Composite_Selected` sheet is added only when a multi-map Composite overlay is selected in the interface.
...
The `examples/` directory contains representative outputs generated with the current InterfaceScout code for the PDB structures used in the manuscript evaluation:
...
Each condition contains only the same user-facing files produced by InterfaceScout: the complete Excel result workbook and the B-factor PDB for the chemistry map used for that example. No intermediate JSON files, auxiliary plots, or analysis-only artifacts are stored in `examples/`.

## Fixed publication settings
