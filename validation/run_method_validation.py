...
def download_first_chain(pid: str) -> Path:
    raw = OUT / f'{pid}_raw.pdb'
    clean = OUT / f'{pid}.pdb'
    if not raw.exists():
        urllib.request.urlretrieve(f'https://files.rcsb.org/download/{pid}.pdb', raw)
    s = PDBParser(QUIET=True).get_structure(pid, str(raw))
...
def score(res, threshold, chem, r1=5.0, r2=8.0, operator='min'):
    ...
    d = distance_matrix(surf)
    defs = ism.CHEMISTRIES[chem]['favorable']
    local = np.zeros(len(surf))
    ...
    def dn(R):
        if not len(surf): return np.zeros(0)
    ...
    return ({r['key']: float(m[i]) for i,r in enumerate(surf)},
            {r['key']: (r['x'],r['y'],r['z']) for r in surf}, (a,b))

def vecmap(a,b):
    keys = sorted(set(a) | set(b))
    return np.array([a.get(k,0.) for k in keys]), np.array([b.get(k,0.) for k in keys])
...
def jacc(a,b,k=10):
    A,B = set(topkeys(a,k)), set(topkeys(b,k))
    return len(A&B)/len(A|B) if A|B else np.nan

def topcenter(a):
    return max(a,key=a.get) if a and max(a.values()) > 0 else None

def center_disp(a,ca,b,cb):
    x,y = topcenter(a), topcenter(b)
    if x is None or y is None: return np.nan
    return float(np.linalg.norm(np.asarray(ca[x]) - np.asarray(cb[y])))

for p in PANEL:
    download_first_chain(p)

conv, rank = [], []
for pid in PANEL:
    by = {n: build(pid,n) for n in POINTS}
    ref = {r['key']:r for r in by[1000]}
    for n in POINTS[:-1]:
        cur = {r['key']:r for r in by[n]}
    ...
    for chem in ism.CHEMISTRIES:
        s200,c200,_ = score(by[200],.05,chem)
        s1000,c1000,_ = score(by[1000],.05,chem)
...
C=pd.read_csv(OUT/'sasa_convergence.csv'); R=pd.read_csv(OUT/'ranking_convergence.csv')
T=pd.read_csv(OUT/'threshold_robustness.csv'); A=pd.read_csv(OUT/'radius_pair_audit.csv'); H=pd.read_csv(OUT/'ph_state_unit_tests.csv')
summary={
 'panel':PANEL,
 'sasa_200_vs_1000':{
