...
def persist_dm(local,dm,pair):
    ns=[]
    for R in pair:
        x=(dm<=R).astype(float)@local
        mx=float(np.max(x)) if x.size else 0.0
        ns.append(x/mx if mx>0 else np.zeros_like(x))
    return 100*np.minimum(ns[0],ns[1])
...
def main():
    ...
    for pid in PANEL:
        ...
        for rep,coords in reps.items():
            base=dmat(coords)
            noises={sig:[dmat(coords+RNG.normal(0,sig,size=coords.shape)) for _ in range(NOISE_REPS)] for sig in NOISE_SIGMAS}
            geom[rep]=(coords,base,noises)
                ...
                for rep,(coords,base_dm,noises) in geom.items():
                    ...
                    for pair in RADIUS_PAIRS:
                        ...
                        for sig in NOISE_SIGMAS:
                            js=[]; rs=[]; dis=[]
                            for ndm in noises[sig]:
                                x=persist_dm(local,ndm,pair)
                                js.append(jac(bt,topk(x)))
                                rr=rho(base,x)
                                if rr is not None: rs.append(rr)
                                xi=int(np.argmax(x)); dis.append(float(np.linalg.norm(bcoord-coords[xi])))
                            vals[sig]=(float(np.median(js)),float(np.median(rs)) if rs else None,float(np.median(dis)))
                        nearby=[]
                        ...
                        raw.append({"pdb":pid,"n_surface":len(rows),"chemistry":chem,"pH":pH,"representation":rep,
                                    "r1":pair[0],"r2":pair[1],
                                    "noise025_top10_jaccard":vals[0.25][0],"noise025_spearman":vals[0.25][1],"noise025_top1_disp_A":vals[0.25][2],
                                    "noise050_top10_jaccard":vals[0.50][0],"noise050_spearman":vals[0.50][1],"noise050_top1_disp_A":vals[0.50][2],
    ...
    for key,rs in groups.items():
        ...
        summary.append({"representation":key[0],"r1":key[1],"r2":key[2],"n_conditions":len(rs),
                        ...
                        "median_radius_neighbor_top10_jaccard":rad,"median_noise025_top1_disp_A":d25,
                        "median_noise050_top1_disp_A":d50,"median_top_patch_members":med("top_patch_compatible_members"),
                        "median_top_patch_diameter_A":med("top_patch_diameter_A"),"audit_rank_score":float(score)})
